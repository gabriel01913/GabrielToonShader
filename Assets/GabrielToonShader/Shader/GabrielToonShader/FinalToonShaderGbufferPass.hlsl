...
half3 ToonColor(InputData inputData, DotData dotInputs, LightInputs lightInputs, Interpolators output)
{
    ...
    #if defined(_ADDITIONAL_LIGHT_REPLACE)    
        half addRamp = saturate(pow(lightInputs.addLightDAtt, 1000));
...
Interpolators vert (Inputs input)
{
    ...
    output.uv02 = input.uv02;
        ...
        #else
        input.normalOS = tex2Dlod(_NormalMap, input.uv0.xy,0,0);
        #endif        
    #endif
...
CustomFragmentOutput frag(Interpolators output)
{    
    half3 color = half3(0,0,0);
    ...
    CustomFragmentOutput buffers = (CustomFragmentOutput)0;
    ...
    #endif
    buffers.gBuffer0 = half4(0,0,0,PackMaterialFlags(materialFlags));
    buffers.gBuffer1 = half4(0,0,0,0);
    buffers.gBuffer2 = half4(output.normalWS, 0);
    buffers.gBuffer3 = half4(color.xyz, baseTint.a);
    return buffers;
}
#endif